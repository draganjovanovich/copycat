...
fn main() {
    ...
    hook.register(KeyCode::AltLeft, move || {
        ...
        // This is debouncer for Alt key, because hook fires twice on key press,
        // and we cannot filter it out in the hook itself.
        if now - alt_timespan_pressed > std::time::Duration::from_millis(500) {
            ...
            if alt_times_pressed == 2 {
                alt_times_pressed = 0;
                std::process::Command::new("/usr/local/bin/popup")
                    .spawn()
                    .unwrap();
            }
        }
